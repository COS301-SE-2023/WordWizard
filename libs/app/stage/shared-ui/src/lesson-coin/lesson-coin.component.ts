  ...
  ngAfterViewInit() {
    setTimeout(() => {
      const element: HTMLElement =
        this.elementRef.nativeElement.querySelector('#level1');
      ...
      this.coins.forEach((coin, index) => {
        if (index < this.coins.length - 1) {
          ...
          const y1 =
            (coin.topPosition || 0) + heightOffset * index + heightOffset / 2;
          const x2 = (this.coins[index + 1].leftPosition || 0) + widthOffset;
          const y2 =
            (this.coins[index + 1].topPosition || 0) +
            heightOffset * (index + 1) +
            heightOffset / 2;
