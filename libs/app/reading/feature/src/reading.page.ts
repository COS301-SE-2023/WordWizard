...
export class ReadingPage {
  ...
  constructor(private store: Store) {
    ...
    this.readingState$.subscribe((data) => {
      this.practice = data;
      if(this.practice.passage.filter(word => word.correct === null).length !== 0)
        this.progressPercentage = `${(this.practice.passage.filter(word => word.correct !== null).length/(this.practice.passage.length*2)) * 100}%`;
    });
