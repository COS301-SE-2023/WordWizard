...
class Rating:
    def __init__(self, vcb, prtc, pref: str, level: int = 10):
        ...
        self.metrices = Metrics(
            self.syllables, self.phono_arr, self.prefixes, self.suffixes
        )
    ...
    def evaluatePhonotactics(self):
        ...
        for phonotactic in phonotactics:
            phonotactic.finalRating = (
                self.child.gameLevel * (4 - get_phonotactic(phonotactic.pattern))
            ) / (
                self.child.readingAbility
                * get_phonotactic(phonotactic.pattern)
                * phonotactic.weight
            )
    ...
    def evaluateSyllables(self):
        ...
        for syllable in syllables:
            syllable.finalRating = (self.child.gameLevel * (5 - syllable.numberOf)) / (
                self.child.readingAbility * (syllable.numberOf) * syllable.weight
            )
    ...
    def evaluatePrefixes(self):
        ...
        for prefix in prefixes:
            prefix.finalRating = (self.child.gameLevel * (4 - prefix.length)) / (
                self.child.readingAbility * (prefix.length) * prefix.weight
            )
    ...
    def evaluateSuffixes(self):
        ...
        for suffix in suffixes:
            suffix.finalRating = (self.child.gameLevel * (4 - suffix.length)) / (
                self.child.readingAbility * (suffix.length) * suffix.weight
            )
