...
def santise_string(response: str):
    ...
    words = [
        Word(word=word.lower() if word.lower() != "i" else word, imageURL="img", correct=None) for word in sentence_arr
    ]
    return_arr = []
    for w in focuse_arr:
        return_arr.append(find_word_index(words, w.strip()))
    for f in return_arr:
        words[f].imageURL = generate_image(f"A cute picture of {words[f].word} for children")
...
def find_phonotactics(word):
    ...
    for pattern, phonotactic in patterns.items():
        ...
        if match and (
            longest_pattern is None or len(match.group()) > len(longest_pattern)
        ):
...
def create(data):
    ...
    with open(
        os.path.join(os.path.dirname(__file__), "data.csv"), "w", newline=""
    ) as csvfile:
...
p = {"CV": 2, "CVC": 3, "CCV": 3, "CVCC": 4, "CCCV": 4, "CCVC": 4, "V": 1, "VC": 2}
